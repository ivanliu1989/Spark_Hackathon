...
def diff_days(s1: String, s2: String, date_format: String, date_unit: Double) = {
...
def generate_features(offer_path: String, transaction_path:String, train_path:String) = {
  // 1 Load and Check the data
  val offers_df = sc.textFile(offer_path).mapPartitionsWithIndex { (idx, iter) => if (idx == 0) iter.drop(1) else iter }.map(_.split(","))
  ...
  // 2 Get all categories and comps on offer in a dict
