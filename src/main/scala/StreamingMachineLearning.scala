...
object StreamingMachineLearning {
  ...
  def main(args: Array[String]) {
    ...
    val testHist_data = testHist_df.mapPartitionsWithIndex { (idx, iter) => if (idx == 0) iter.drop(1) else iter }
                                   .map(_.split(","))
                                   .map(r=>testHist(r(0),r(1).toInt,r(2),r(3),r(4))).toDF()
    ...
    val trainHist_data = trainHist_df.mapPartitionsWithIndex { (idx, iter) => if (idx == 0) iter.drop(1) else iter }
                                   .map(_.split(","))
                                   .map(r=>trainHist(r(0),r(1).toInt,r(2),r(3),r(4).toInt,r(5),r(6))).toDF()
                               
    trainHist_data.registerTempTable("trainHist_table") 
    
    // SQL Table transactions
    case class transactions(id: String, chain: Int, dept: String, category: String, company: String, brand: String, date: String, 
                            productsize: String, productmeasure: String, purchasequantity: Int, purchaseamount: Double)
    val transactions_data = transactions_df.mapPartitionsWithIndex { (idx, iter) => if (idx == 0) iter.drop(1) else iter }
                                           .map(_.split(","))
                                           .map(r=>transactions(r(0),r(1).toInt,r(2),r(3),r(4),r(5),r(6),r(6),r(7),r(8),r(9).toInt,r(10).toDouble)).toDF()
                               
    transactions_data.registerTempTable("transactions_table") 
    
    // SQL Query
    sqlContext.sql("Select * from offers_table").show
